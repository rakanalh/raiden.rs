...
use reqwest;
use crate::primitives::EnvironmentType;

use super::constants::{MATRIX_DEFAULT_DEVELOPMENT_SERVERS_LIST_URL, MATRIX_DEFAULT_PRODUCTION_SERVERS_LIST_URL};

pub async fn get_default_matrix_servers(environment_type: EnvironmentType) -> reqwest::Result<Vec<String>> {
    ...
    Ok(servers.iter().map(|s| {
        if s.starts_with("http") { return s.clone() }
        else { format!("https://{}", s) }
    }).collect())
}
