...
use web3::types::{Address, Filter, FilterBuilder, U64};
...
impl ContractRegistry {
    pub fn new(chain_id: ChainID) -> Result<Self, ethabi::Error> {
        ...
        let contracts_map = contracts_data
			.get("contracts")
			.unwrap()
			.as_object()
			.unwrap();

		let token_network_registry = Self::get_contract(
	...
	fn get_contract(name: &'static str, data: Map<String, Value>) -> Result<Contract, ethabi::Error> {
		...
		let address = match contract_deployment_map.get("address").unwrap().as_str().unwrap().trim_start_matches("0x").parse() {
			Ok(value) => value,
			Err(_) => Address::zero(),
		};
	...
	pub fn add_token_network(&mut self, address: Address, block_number: U64) -> Result<Contract, ethabi::Error> {
		...
		match self.contracts.get_mut(&ContractIdentifier::TokenNetwork) {
			...
			None => {
				self.contracts.insert(ContractIdentifier::TokenNetwork, vec![contract.clone()]);
			},
		};
		Ok(contract)
	}
