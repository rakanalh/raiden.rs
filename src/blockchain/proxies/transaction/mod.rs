...
pub trait Transaction {
    ...
    async fn estimate_gas(&self, params: Self::Params, data: Self::Data) -> Result<(U256, U256), ()>;

    async fn execute(&self, params: Self::Params, at_block_hash: H256) -> Result<Self::Output, ProxyError> {
        ...
        if let Ok(_) = self.validate_postconditions(params.clone(), at_block_hash).await {
            if let Ok((gas_estimate, gas_price)) = self.estimate_gas(params.clone(), data.clone()).await {
