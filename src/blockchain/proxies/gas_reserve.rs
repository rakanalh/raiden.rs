...
impl GasReserve {
    pub fn new(proxy_manager: Arc<ProxyManager>, registry_address: Address) -> Self {
        Self {
            gas_metadata: proxy_manager.gas_metadata(),
            proxy_manager,
            registry_address,
        }
    }
    fn gas_required_for_channel_lifecycle_after_close(&self) -> u64 {
        self.gas_metadata.get("TokenNetwork.settleChannel") + GAS_REQUIRED_FOR_CHANNEL_LIFECYCLE_AFTER_SETTLE
    }

    fn gas_required_for_channel_lifecycle_after_open(&self) -> u64 {
        self.gas_metadata.get("TokenNetwork.closeChannel") + self.gas_required_for_channel_lifecycle_after_close()
    }
    ...
    async fn get_required_gas_estimate(&self, chain_state: &ChainState, channels_to_open: u64) -> Result<u64> {
        ...
        for token_address in token_addresses {
            num_opened_channels +=
                views::get_channelstate_open(&chain_state, self.registry_address, token_address).len() as u64;
            num_closing_channels +=
                views::get_channelstate_closing(&chain_state, self.registry_address, token_address).len() as u64;
            num_closed_channels +=
                views::get_channelstate_closed(&chain_state, self.registry_address, token_address).len() as u64;
            num_settling_channels +=
                views::get_channelstate_settling(&chain_state, self.registry_address, token_address).len() as u64;
            num_settled_channels +=
                views::get_channelstate_settled(&chain_state, self.registry_address, token_address).len() as u64;
        }
    ...
    pub async fn get_estimate(&self, chain_state: &ChainState, channels_to_open: u64) -> Result<U256> {
    ...
    pub async fn has_enough(&self, chain_state: &ChainState, channels_to_open: u64) -> Result<(bool, U256)> {
        let gas_reserve_estimate = self.get_estimate(chain_state, channels_to_open).await?;
        let balance = self
            .proxy_manager
            .web3()
            .eth()
            .balance(self.proxy_manager.account().address(), None)
            .await?;
