use crate::{
	...
	state_machine::{
		types::{
			ActionInitTarget, Block, ChainState, ChannelState, ContractReceiveSecretReveal,
			ErrorUnlockClaimFailed, Event, PaymentReceivedSuccess, ReceiveLockExpired,
			ReceiveSecretReveal, ReceiveUnlock, SendMessageEventInner, SendSecretRequest,
			SendSecretReveal, StateChange, TargetState, TargetTransferState,
		},
		views,
	},
};
...
use super::{
	channel, mediator, secret_registry,
...
fn handle_init_target(
	...
	let mut channel_state = match views::get_channel_by_canonical_identifier(
		...
		None => {
			return Ok(TargetTransition { new_state: None, chain_state, events: vec![] })
		},
	};
