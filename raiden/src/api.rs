...
use crate::{
	blockchain::{
		...
		proxies::{Account, GasReserve, ProxyError},
	},
	constants::{self, ABSENT_SECRET, SECRET_LENGTH},
	errors::StateTransitionError,
	pathfinding::RoutingError,
	payments::PaymentsRegistry,
	primitives::{
		BlockTimeout, ChannelIdentifier, PaymentIdentifier, RetryTimeout, RevealTimeout, Secret,
		SecretHash, SecretRegistryAddress, SettleTimeout, TokenAddress, TokenAmount,
	...
	state_machine::{
		types::{
			ActionChannelSetRevealTimeout, ActionInitInitiator, ChannelState, ChannelStatus,
			RouteState, TransferDescriptionWithSecretState,
		},
		views,
	},
	utils::{random_identifier, random_secret},
	waiting,
};
...
impl Api {
	...
	pub async fn create_channel(
		...
		let settle_timeout =
			settle_timeout.unwrap_or(SettleTimeout::from(constants::DEFAULT_SETTLE_TIMEOUT));
		let reveal_timeout =
			reveal_timeout.unwrap_or(RevealTimeout::from(constants::DEFAULT_REVEAL_TIMEOUT));
