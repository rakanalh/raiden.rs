...
use crate::{
	pathfinding::{self, PFSPath, RoutingError, PFS},
	primitives::{
		AddressMetadata, BlockNumber, ChannelIdentifier, OneToNAddress, PFSConfig, PrivateKey,
		TokenAmount, TokenNetworkAddress,
	},
	state_machine::{
		types::{ChainState, ChannelState, ChannelStatus, RouteState},
		views,
	},
};
	...
	if token_network.partneraddresses_to_channelidentifiers.contains_key(&to_address) {
		for channel_id in token_network.partneraddresses_to_channelidentifiers[&to_address].iter() {
			...
			if channel_state.is_usable_for_new_transfer(payment_with_fee_amount, None) {
				...
				let metadata =
					pathfinding::query_address_metadata(pfs_config.url, to_address).await?;
