...
use crate::{
	...
	errors,
	primitives::{ChainID, TokenNetworkRegistryAddress, U64},
	state_machine::{
		machine::chain,
		types::{
			ActionInitChain, ChainState, ContractReceiveTokenNetworkRegistry, Event, StateChange,
...
impl StateManager {
	...
	fn dispatch(&mut self, state_change: StateChange) -> Result<Vec<Event>> {
		...
		match chain::state_transition(current_state, state_change) {
			...
			Err(e) =>
				Err(errors::StateTransitionError { msg: format!("Could not transition: {}", e) }),
		}
	}
