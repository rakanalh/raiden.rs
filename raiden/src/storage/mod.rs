...
use crate::{
	errors::{RaidenError, TypeError},
	state_machine::types::{ChainState, Event, StateChange},
};
...
impl Storage {
	...
	pub fn store_snapshot(&self, state: ChainState, state_change_id: Option<Ulid>) -> Result<()> {
		let serialized_state =
			serde_json::to_string(&state).map_err(|_| StorageError::SerializationError)?;
		let sql = format!(
			"
	...
	pub fn store_events(&self, state_change_id: Ulid, events: Vec<Event>) -> Result<()> {
		let serialized_events =
			serde_json::to_string(&events).map_err(|_| StorageError::SerializationError)?;
		let sql = format!(
